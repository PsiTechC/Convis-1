...
export default function VoiceLabPage() {
  ...
  const fetchVoices = async (filters?: { provider?: string; gender?: string; accent?: string }) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/voices/list?${params.toString()}`,
        {
          headers: {
  ...
  const fetchSavedVoices = async (uid: string) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/voices/preferences/${uid}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
  ...
  const handleSaveVoice = async (voice: Voice) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/voices/preferences/${userId}/save`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  const handleRemoveVoice = async (voice: Voice) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/voices/preferences/${userId}/remove`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  const handlePlayVoice = async (voice: Voice) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/voices/demo`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
            ...
            {filteredVoices.map((voice) => {
              ...
              return (
                <div
                  ...
                  className={`rounded-xl shadow-sm border-2 p-6 hover:shadow-md transition-all duration-200 ${
                    isDarkMode
                      ? isSaved
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-lg">{genderIcons[voice.gender]}</span>
                      </div>
                  ...
                  {/* Description */}
                  {voice.description && (
                    <p className={`text-sm mb-4 ${isDarkMode ? 'text-gray-300' : 'text-neutral-mid'}`}>
                      {voice.description}
                    </p>
                  )}
